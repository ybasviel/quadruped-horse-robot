...
void GAIT::walk(){
  ...
  const double offset[4] = {3*M_PI/2.0, M_PI/2.0, 2*M_PI/2.0, 0};

  double yf[4] = {0 + gyroY/10000.0, -2 - gyroY/10000.0, 0 + gyroY/10000.0, -2 - gyroY/10000.0};

  for(uint8_t i = 0; i < 4; i++){
    x = 10 * cos( -(t + offset[i]) );
    y = 80 + 3 * sin( -(t + offset[i]) ) + yf[i];
...
void GAIT::step(){ 
  ...
  double yf[4] = {0 + gyroY/10000.0, - gyroY/10000.0, 0 + gyroY/10000.0, - gyroY/10000.0};
...
void GAIT::turn(){
  ...
  double yf[4] = { 0 + gainY, -1 - gainY, 0 + gainY, -1 - gainY };
  double zf[4] = { 0, -3, 0, -3 };

  for(uint8_t i = 0; i < 4; i++){
    x = (12  + zf[i]) * powOfMinus1(i)*cos( -(t + offset[i]) );
    y = 80 + 10 * sin( (t + offset[i]) );
